...
class DStackManager:
    ...
    def run_instance(self, vm_dir: str, host_port: int, imgdir: Optional[str] = None, dry_run: bool = False) -> None:
        ...
        base_args = [
            ...
            '-smp', str(vcpu_count),
            '-cpu', 'host',
            '-machine', 'q35,kernel_irqchip=split,confidential-guest-support=sev,memory-encryption=sev,hpet=off',
            '-object', 'sev-guest,id=sev,policy=0x1,cbitpos=47,reduced-phys-bits=5',
            '-nographic',
            '-nodefaults',

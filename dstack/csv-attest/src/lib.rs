...
impl CsvAttestationClient {
    ...
    pub fn verify_certificate_chain(&self, report: &CsvAttestationReport) -> CsvAttestationResult<()> {
        ...
        let ret = unsafe {
            validate_cert_chain(report_bytes.as_ptr() as *const csv_attest_sys::CsvAttestationReport)
        };
    ...
    /// 验证 HRK 证书
    pub fn verify_hrk_certificate(&self, hrk_cert: &csv_attest_sys::ChipRootCert) -> CsvAttestationResult<()> {
        let ret = unsafe {
            verify_hrk_cert_signature(hrk_cert as *const csv_attest_sys::ChipRootCert)
        };
    ...
    /// 验证 HSK 证书
    pub fn verify_hsk_certificate(&self, hrk_cert: &csv_attest_sys::ChipRootCert, hsk_cert: &csv_attest_sys::ChipRootCert) -> CsvAttestationResult<()> {
        let ret = unsafe {
            verify_hsk_cert_signature(
                hrk_cert as *const csv_attest_sys::ChipRootCert,
                hsk_cert as *const csv_attest_sys::ChipRootCert
            )
        };
    ...
    /// 验证 CEK 证书
    pub fn verify_cek_certificate(&self, hsk_cert: &csv_attest_sys::ChipRootCert, cek_cert: &csv_attest_sys::CsvCert) -> CsvAttestationResult<()> {
        let ret = unsafe {
            verify_cek_signature(
                hsk_cert as *const csv_attest_sys::ChipRootCert,
                cek_cert as *const csv_attest_sys::CsvCert
            )
        };
    ...
    /// 验证 PEK 证书
    pub fn verify_pek_certificate(&self, cek_cert: &csv_attest_sys::CsvCert, pek_cert: &csv_attest_sys::CsvCert) -> CsvAttestationResult<()> {
        let ret = unsafe {
            verify_pek_cert_with_cek_signature(
                cek_cert as *const csv_attest_sys::CsvCert,
                pek_cert as *const csv_attest_sys::CsvCert
            )
        };
    ...
    pub fn verify_report_signature(&self, report: &CsvAttestationReport) -> CsvAttestationResult<()> {
        ...
        let ret = unsafe {
            csv_attestation_report_verify(report_bytes.as_ptr() as *const csv_attest_sys::CsvAttestationReport)
        };
    ...
    pub fn load_hrk_certificate(&self, filename: &str) -> CsvAttestationResult<Vec<u8>> {
        ...
        let mut buffer = vec![0u8; std::mem::size_of::<csv_attest_sys::ChipRootCert>()];
    ...
    pub fn load_hsk_cek_certificates(&self, chip_id: &str) -> CsvAttestationResult<(Vec<u8>, Vec<u8>)> {
        ...
        let mut hsk_buffer = vec![0u8; std::mem::size_of::<csv_attest_sys::ChipRootCert>()];
        let mut cek_buffer = vec![0u8; std::mem::size_of::<csv_attest_sys::CsvCert>()];

...
static int verify_pek_cert(CSV_CERT_t * verifier, CSV_CERT_t *pek)
{
    ...
    key_usage_t key_type;

    ecc_pubkey_t *pubkey = &verifier->ecc_pubkey;

    key_type = verifier->pubkey_usage;

    if (key_type == KEY_USAGE_TYPE_CEK)
        signature = &pek->ecc_sig1;
    else if (key_type == KEY_USAGE_TYPE_OCA)
        signature = &pek->ecc_sig2;
    else {
        printf("invalid key type %d, PEK cert only support CEK or OCA signature\n", key_type);
        return -1;
    }
...
{
    int ret = -1;
    userid_u *userid = (userid_u*)oca->ecc_pubkey.user_id;

    logcat("\nOCA user id: %s\n", userid->uid);

    ret = verify_pek_cert(oca, pek);
...
int get_hrk_cert(char *cert_file)
{
    ...
    if (cmd_ret != 0) {
        /* check if there is offline cert */
...
int validate_cert_chain(struct csv_attestation_report *report){
    ...
    do {
        ...
        if (external_oca_file) {
            ret = verify_pek_cert_with_oca_signature(&oca, &g_pek_cert);
            if(ret){
                logcat("oca pubkey and verify pek cert failed\n");
                break;
            }
            logcat("oca pubkey verify pek cert successful\n");
        }
...
int convert_ecc_point_to_EC_KEY(struct ecc_point_q *coordinate, EC_KEY * eckey)
{
    EC_GROUP *group = NULL;
    EC_POINT *point = NULL;
    BIGNUM   *x     = NULL;
    ...
    if (group == NULL) {
        logcat("Failed to create EC group\n");
        goto finish;
    }
    ...
    if (!BN_bin2bn(coordinate->Qx, ECC_LEN, x) || !BN_bin2bn(coordinate->Qy, ECC_LEN, y)) {
        logcat("Failed to convert coordinate to BIGNUM\n");
        goto finish;
    }

    if (!EC_POINT_set_affine_coordinates_GFp(group, point, x, y, NULL)) {
        logcat("Failed to set affine coordinates\n");
        goto finish;
    }

    if (!EC_KEY_set_group(eckey, group)) {
        logcat("fail to set pubkey to group\n");
        goto finish;
    }
...
int  save_csv_pubkey_to_pem_file(ecc_pubkey_t *pubkey, char *pemfile)
{
    ...
    if (!pubkey || !pemfile) {
        logcat("invalid pubkey or invalid pem file name\n");
        goto finish;
    }

    fd = open(pemfile, O_CREAT|O_WRONLY|O_TRUNC, 0644);
    if (fd < 0) {
        printf("open file %s fail %d, error (%s)\n", pemfile, fd, strerror(errno));
        goto finish;
    }
    ...
    eckey = EC_KEY_new();
    if ( convert_ecc_point_to_EC_KEY(&Q, eckey)) {
        printf("fail to save pubkey coordinate to EC pubkey\n");
        goto finish;
    }

    if (!PEM_write_EC_PUBKEY(fp, eckey)) {
        printf("fail to write EC pubkey to file %s\n", pemfile);
        goto finish;
    }
